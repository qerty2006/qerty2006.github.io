...
With my differential equations midterm coming soon (literally next week), I wanted to come back to the basics, and go over all of the stuff that I learned over the last 8 weeks, hopefully in a format that will allow anyone with a basic understanding of calculus can follow along.
...
## Exact Differential Equation: $M(x,y)dx + N(x,y)dy = 0 \quad | \quad M_y = N_x$
...
$$ \int M(x,y) dx = xy + C_1 \quad \text{and} \quad \int N(x,y) dy = y^3 + xy + C_2 $$

Solving for $u(x,y)$ and removing the duplicate term $xy + C$ , we get

$$ u(x,y) = xy + y^3 + C $$


## Linear: $\frac{dy}{dx} + g(x)y = f(x)$
...
## Autonomous: $\frac{dy}{dx} = f(y)$ (x doesn't affect the slope)
...
- Arrows indicate the direction of the flow (i.e., increase or decrease of $y$) between equilibrium points.
- Stability is visually represented by the direction of arrows converging towards or diverging from equilibrium points.
...
## Second Order Constant Coefficient Differential Equations: $ay + by' + cy'' = 0$
...
## Second Order Undetermined Coefficient Differential Equations: $a(x)y + b(x)y' + c(x)y'' = d(x)$
...
This indicates a need to adjust our assumption due to overlap with the homogeneous solution. Try:
...
## System of Differential equations (Matrix Time!):  $y' = A(x,y)y$
